...
class Trainer:
    def __init__(self, forward, inverse, records):
        ...
        self.epoch = 100
        # 0 if not update
        self.threshold = record['err_test'][-1] if load else 1

        train_numpy = np.array(np.load(
            r'C:\Users\86187\Documents\GitHub\Weiczh-s-1st\hw3\data\AI_train.npy'), dtype=np.float32)
        ...
        test_numpy = np.array(np.load(
            r'C:\Users\86187\Documents\GitHub\Weiczh-s-1st\hw3\data\AI_test.npy'), dtype=np.float32)
    ...
    def train(self):
        optimizer = optim.Adam(
            self.inverse_net.parameters(), lr=self.learning_rate)
        ...
        for t in range(self.epoch):
            for step, item in enumerate(self.train_loader):
                ...
                if self.iteration % self.interval == 0:
                    ...
                    # test
                    test_predict = self.forward_net(
                        self.inverse_net(self.test_input))
                    ...
                    print('iteration: {}, time: {}'.format(
                        self.iteration, end-start))
    ...
    def inference(self, c):
        ...
        spectrum_predict = to_numpy(
            self.forward_net(intermediate).view(-1)) * 100
        ...
        plt.plot(spectra, structure_predict, color=color1,
                 label='Prediction', marker='o')
        plt.plot(spectra, structure_label, color=color2,
