...
class Trainer:
    def __init__(self, generator, discriminator, forward_net, records):
        ...
        train_numpy = np.array(
            np.load(r'C:\Users\86187\Documents\GitHub\Weiczh-s-1st\hw3\data\AI_train.npy'), dtype=np.float32)
    ...
    def train_loop(self):
        ...
        def get_fitted_model(x, obj, state_dict=None):
            ...
            mll = ExactMarginalLogLikelihood(
                fitted_model.likelihood, fitted_model)
            mll.to(x)
        ...
        for iteration in range(N_BATCH):
            ...
            qEI = qExpectedImprovement(
                model=model, sampler=qmc_sampler, best_f=standardize(train_obj).max())
        ...
        for t in range(self.epoch):
            for step, item in enumerate(self.train_loader):
                ...
                noise_data = np.random.uniform(
                    high=1, low=-1, size=(self.batch_size, 10))
                ...
                if i % (self.critic + 1) != 0:
                    ...
                    gp = self.gradient_penalty(
                        train_real.data, train_fake.data, train_label.data)
                ...
                if i % per == 0:
                    ...
                    # compute test error
                    noise_data = np.random.uniform(
                        high=1, low=-1, size=(self.test_label.shape[0], 10))
                    noise_tensor = torch.FloatTensor(noise_data).to(device)
